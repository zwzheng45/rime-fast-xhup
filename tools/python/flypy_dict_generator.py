...
def gen_dict_record(pinyin_list, per_line_contents, *args):
    if not pinyin_list:
        return None
    ...
    if shape_type in ["hxm", "zrm"] and pinyin_style != "jianpin":  # 转换对应汉字的形码
        # words_xm_list = [zrxm_dict.get(m, "[") for m in per_line_contents[0].strip()]
        words_xm_list = [xhxm_dict.get(m, "|") for m in per_line_contents[0].strip()]
    ...
    if int(word_freq) > 1:  # 当指定文件为 yaml 或词频大于1
        word_frequency = f"{word_freq}"
    elif filetype == "yaml":
        word_frequency = per_line_contents[-1] if per_line_contents[-1].isnumeric() else word_freq
    else:
        word_frequency = word_freq or ""

    print("pinyin_list: ", pinyin_list, word_freq)
    return f"{per_line_contents[0].strip()}\t{yxencode_str}\t{word_frequency}\n"


def parser_line_content(line_content, *args):
    per_line_contents = line_content.strip().split("\t")
    if not per_line_contents[0]:
        return None

    pinyin_style, need_convert = args[0], args[1]
    if need_convert:
        pinyin_list = lazy_pinyin(per_line_contents[0])
    elif pinyin_style == "jianpin":
        pinyin_list = lazy_pinyin(per_line_contents[0], style=Style.FIRST_LETTER)
    else:
        pinyin_list = per_line_contents[1].split() if len(per_line_contents) > 1 else None

    yield gen_dict_record(pinyin_list, per_line_contents, *args)
...
def get_cli_args():
    ...
    parser.add_argument(
        ...
        help="spec from chinese convert to pinyin style",
        default=False,
    )
    parser.add_argument(
        "--shape",
...
def main():
    ...
    print(
        ...
        f"convert_to_pinyin: {cli_args.chinese_to_pinyin}\n",
        """
