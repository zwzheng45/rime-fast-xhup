...
此分支单纯自用，本人对Lua一窍不通，对Rime也是一知半解，只是为了找一个好用的小鹤音形方案才入坑了Rime，发现了boomker大佬的方案非常适合（大佬受我一拜Orz）。我稍微做了些适合自己需求的改动，目前本分支不会跟进主仓库（看不懂Lua思密达），只会自动~~剽窃~~更新主仓库的词典文件和搜狗词典文件。
