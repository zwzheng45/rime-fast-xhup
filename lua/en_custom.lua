...
-- 如果用户文件夹中找到了dict_name指向的词库文件，则将文件目录赋值给dict_dir
if io.open(user_data_dir .. dict_name, "r") then
	dict_path = user_data_dir .. dict_name
else
	-- 如果用户文件夹中没找到dict_name指向的词库文件，但是程序文件夹中找到了，则将文件目录赋值给dict_dir
...
-- 输入字符串和文件路径，在路径指向文件中逐行查找完全对应的字符串，如果已存在则返回true，适用于小狼毫判断词条是否已经存在
local function user_dict_exist(str, path)
